...
const MultiSelectFun = () => {
  ...
  return (
    <div className="bg-[#eef1f8] h-screen grid place-items-center ">
      <div className="relative w-9/12 h-96 text-sm">
        {selected?.length ? (
        ...
        {/* Input */}
        <div className="bg-white border flex flex-nowrap p-3  gap-2.5">
          {/* <Icons.Search /> */}
          <input
            ref={inputRef}
            type="text"
            ...
            className="bg-transparent text-lg outline-none flex-1 w-9/12 caret-rose-600 pl-5"
          ...
          />
          <button
            className="text-sm disabled:text-gray-300 text-rose-500 w-24 disabled:cursor-not-allowed"
            disabled={isDisable}
            onClick={() => {
              if (isDisable) {
                return;
